...
def get_counter_kb(exercise_id, measerment: str = 'numbers'):
    ...
    keys = [Ibtn(text, callback_data=f'counter {exercise_id} {callback}') for text, callback in counter_set[measerment].items()]
    keys.append(Ibtn('Своё', callback_data=f'counter {exercise_id} another'))
